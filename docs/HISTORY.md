...
2026-08-06 (s306): `list_stubs` display-limit trap — FIXED. The SQL `LIMIT 20` was applied before the Python code split FSM vs non-FSM stubs. On dj2 (12 FSM stubs), 2 subrace stubs were silently cut. Fix: removed LIMIT from SQL, applied `regular_rows[:limit]` after split so FSM stubs always show in full and the limit only constrains non-FSM stubs.
