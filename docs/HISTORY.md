...
2026-08-06 (s306): `frontier_priority` and `_get_chain_positions` had the same Class.method caller JOIN bug as `_caller_names` in s305. SQL `JOIN functions caller_fn ON caller_fn.name = ge.caller` failed for callers stored as "ContextBuilder.build" (no such row in functions; bare "build" is what's stored). Result: production stubs with Class.method callers were invisible to frontier_priority entirely — not scored, not in any chain set. Fix: bare-name + caller_file fallback added to all three SQL JOINs (frontier_priority direct-call query, has_functional_caller, has_stub_caller in _get_chain_positions).

2026-08-06 (s306): `list_stubs` display-limit trap. The SQL `LIMIT 20` is applied before the Python code splits FSM vs non-FSM stubs. For corpora with many FSM stubs (dj2 has 12), FSM stubs consume most of the 20 slots, leaving few slots for Python stubs. On dj2, 2 subrace stubs (`subraces`, `semantic_match_fighting_style`) are silently cut. The DB has 25 stubs (22 non-test); only 20 are shown. Fix: apply limit separately per section, or raise the default limit to account for FSM stubs.

2026-08-05 (s305): `_caller_names` in list_stubs had a false-unresolved bug. It matched caller names with `f2.name = ge.caller` (exact), but graph_edges stores callers as `ContextBuilder.build` while functions stores bare `build`. All Class.method callers appeared as "(unresolved)" even when the method existed in the corpus. Session 298 concluded "phantom edges, lower priority" for _get_encounter_context, _get_combat_context, semantic_match_subrace, on_arc_completed — that was wrong. Those stubs have real implemented callers. Fix: bare-name fallback using caller_file disambiguation.
