...
|--------|--------|--------|
| Determined (Python) | Full convergence | probe 2026-08-05 (fresh re-ingest): 1 real stub (suggest_tags, known accepted), 9 test mocks, 0 false positives; prior "real gaps" (pattern_executor.__init__, contract_drift_classifier.__init__) were phantom detections in stale July 17 DB — cleared by fresh ingest, neither class has __init__; ABC gaps: clean; 95.4% unresolved edges (external-lib ceiling, accepted); 587 EPs in determined/ (0 stubs); docstring health 15.0% missing in core (non-test, non-example) — 191/1277 after s303-s304 batches; all 3 convergence criteria met |
| dj2 (Python+JS) | Full convergence | probe 2026-08-06 (post s306 fixes): 25 stubs — 12 FSM (accepted), 3 test (accepted), 5 subrace/RM68 delete candidates (subraces, get_subraces_for_race, get_race_for_subrace, semantic_match_subrace, semantic_match_fighting_style), 5 production gaps (_get_combat_context, _get_encounter_context, on_arc_completed, process_consequences, _register_world_tools); frontier_priority now correctly surfaces all 5 production stubs (Class.method caller JOIN bug fixed); list_stubs display-limit bug fixed (FSM stubs no longer crowd out Python stubs; all 10 non-FSM stubs now visible); 8 phases.py ABCs = 39 intentional scaffold methods, 0 concrete gaps; unresolved edge ratio 87.9% (accepted ceiling); docstring health 56.0% (809/1444 missing); all 3 convergence criteria met; s308: list_features EP definition fixed (edges→distinct symbols) — world/ 164→39 EP (164 CrossEdges), world_app.py 86 EP/160 edges, dungeon_neo 6 EP/6 edges (confirmed isolated); EP and CrossEdges now meaningfully distinct columns |
