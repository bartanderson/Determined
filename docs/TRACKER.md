...
| dj2 (Python+JS) | Full convergence | probe 2026-08-06 (post frontier_priority fix): 25 stubs — 12 FSM (accepted), 3 test (accepted), 5 subrace/RM68 delete candidates (subraces, get_subraces_for_race, get_race_for_subrace, semantic_match_subrace, semantic_match_fighting_style), 5 production gaps (_get_combat_context, _get_encounter_context, on_arc_completed, process_consequences, _register_world_tools); frontier_priority now correctly surfaces all 5 production stubs (was hiding them due to Class.method caller JOIN bug); 8 phases.py ABCs = 39 intentional scaffold methods, 0 concrete gaps; unresolved edge ratio 87.9% (accepted ceiling); docstring health 56.0% (809/1444 missing); all 3 convergence criteria met |
