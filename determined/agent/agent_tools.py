...
from determined.agent.export_context import (  # noqa: E402
    export_context, export_context_append, export_context_dump,
)
TOOLS["export_context"]        = (export_context,        "assessor")
TOOLS["export_context_append"] = (export_context_append, "assessor")
TOOLS["export_context_dump"]   = (export_context_dump,   "assessor")
...
def _primer_items(oracle, top_n: int = 5) -> list[dict]:
    ...
    for name, file_path, docstring, line_no in python_stubs:
        ...
        # Skip intentional scaffold stubs: empty body, no concept signals, ≤1 caller
        if (signals.get("body_shape") == "empty_pass"
                and not signals.get("concept_presence")
                and caller_count <= 1):
            continue
    ...
    # Annotate Python stubs that are blocked by an FSM in the list.
    # If a stub's purpose mentions an FSM name that already appears above it, add blocked_by.
    fsm_rank = {item["name"]: i + 1 for i, item in enumerate(result) if item["is_fsm"]}
    for i, item in enumerate(result):
        if item["is_fsm"]:
            continue
        purpose_lower = (item.get("purpose") or "").lower()
        for fsm_name, rank in fsm_rank.items():
            if rank <= i and fsm_name.lower() in purpose_lower:
                item["blocked_by"] = {"rank": rank, "name": fsm_name}
                break
...
def fsm_scaffold(assessor: "Assessor", args: dict) -> str:
    """
    fsm_scaffold(fsm_name) — generate Python handler stub module for all actions and guards
    in the named FSM.

    Reads the FSM stubs from the DB, emits one def per action/guard with its docstring.
    Deterministic — no LLM.  Output is ready to paste into a new Python module.

    Args:
      fsm_name   — exact FSM name (e.g. "EncounterFSM")
    """
    fsm_name = args.get("fsm_name", "").strip()
    if not fsm_name:
        return "Error: fsm_name is required."

    conn = assessor.oracle.conn
    rows = conn.execute(
        "SELECT name, docstring FROM functions "
        "WHERE is_stub=1 AND (name LIKE ? OR name LIKE ?)",
        (f"{fsm_name}::action::%", f"{fsm_name}::guard::%"),
    ).fetchall()

    if not rows:
        return f"No stubs found for FSM '{fsm_name}'."

    actions = [(n.split("::")[2], d or "") for n, d in rows if "::action::" in n]
    guards  = [(n.split("::")[2], d or "") for n, d in rows if "::guard::" in n]

    lines = [
        f'"""',
        f"Handler module for {fsm_name}.",
        f"Generated by fsm_scaffold — fill in each function body.",
        f'"""',
        "",
        "",
    ]
    ...
    if actions:
        ...
        for fn_name, doc in sorted(actions):
            lines.append(f"def {fn_name}(context):")
            if doc.strip():
    ...
    if guards:
        lines.append("")
        lines.append("# ── Guards ───────────────────────────────────────────")
        for fn_name, doc in sorted(guards):
            lines.append(f"def {fn_name}(context) -> bool:")
            if doc.strip():
                first = doc.strip().split("\n")[0]
                lines.append(f'    """{first}"""')
