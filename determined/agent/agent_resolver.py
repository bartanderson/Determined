...
_PATTERNS = [
    ...
    (re.compile(r"(?:cross.?language\s+calls?|cross.?layer\s+calls?|js\s+to\s+python|socket(?:\.io)?\s+(?:calls?|handlers?)|htmx\s+routes?)(?:\s+(?:for|in|involving)\s+['\"]?([^'\"]*?)['\"]?)?\s*$", re.I),
     "find_cross_language_calls", "scope", 1),
